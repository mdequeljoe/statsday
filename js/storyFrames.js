...
//m1 frame events
function m1_f1(){
    drawChords(m1) //can take out later since already drawn in intro - only for testing
    clearText();
...
function m5(){
    clearProgressBars(20 * 1000)
    clearText();
    setText("#label", story.frame5.label, 250)
    setLink("#hi", story.frame5.step1, story.frame5.link1Text, story.frame5.link1)
    setLink("#mid", story.frame5.step2,story.frame5.link2Text, story.frame5.link2)
    setLink("#low", story.frame5.step3,story.frame5.link3Text, story.frame5.link3)
}
...
function progressBar(id, t){
  ...
  function setLink(id, text, delay, t){
    if (typeof t === "undefined") t = 1000;
    if (typeof delay === "undefined") delay = 250;
    var t0 = d3.select(id)
    .transition()
  ...
  function resetAll(delay, t){
    ...
    for (var i = 0; i < highlighted.length; i++){
      var idno = highlighted[i].match(/[0-9]/)[0],
      fill = idno % 2 == 0 ? '#00AACC': '#FDAF18'
  ...
  function setLink(id, text, linkText, link, delay, t){
      ...
      document.getElementById(id.replace("#", "")).style.opacity = "0"
